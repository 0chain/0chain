...
docker run zchain_unit_test bash -c '(cd 0chain.net; go install github.com/golangci/golangci-lint/cmd/golangci-lint@v1.58.1; golangci-lint run --build-tags bn256 --timeout 10m0s --exclude-files ".*_test\.go$" --out-format tab)'

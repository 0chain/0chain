...
cmd="build"
dockerfile="docker.local/build.benchmarks/Dockerfile"
platform=""
...
then
    ...
    docker $cmd -f $dockerfile . -t zchain_benchmarks
else
    PACKAGE="$1"
...
if [[ -n "$PACKAGE" ]]; then
    ...
    docker run "$INTERACTIVE" zchain_benchmarks sh -c "cd /0chain/code/go/$PACKAGE; go test -tags bn256 ./..."
else
    # Run all tests.
    echo "Running general unit tests:"
    docker run "$INTERACTIVE" -v $(pwd)/code:/codecov zchain_benchmarks sh -c "cd 0chain.net; go test -tags bn256 -coverprofile=/codecov/coverage.txt -covermode=atomic ./..."
fi

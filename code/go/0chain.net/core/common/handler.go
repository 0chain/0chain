...
func Respond(w http.ResponseWriter, r *http.Request, data interface{}, err error) {
	if err != nil {
		...
		switch {
		case zchainErrors.Is(err, ErrBadRequest):
			w.WriteHeader(http.StatusBadRequest)
		case zchainErrors.Is(err, ErrInternal):
			w.WriteHeader(http.StatusInternalServerError)
		case zchainErrors.Is(err, ErrNoResource):
...
func JSONString(json map[string]interface{}, field string, required bool) (string, error) {
	...
	if !ok {
		if required {
			return "", zchainErrors.Newf("", "input %v is required", field)
		}
		return "", nil
	}

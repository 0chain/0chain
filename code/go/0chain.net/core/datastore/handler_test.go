...
func TestPrintEntityHandler(t *testing.T) {
	type args struct {
		ctx    context.Context
	...
	tests := []struct {
		...
		{
			name: "Test_PrintEntityHandler_OK",
			args: args{
				ctx:    context.TODO(),
		...
		{
			name: "Test_PrintEntityHandler_ERR",
			args: args{
				ctx:    context.TODO(),
				entity: &block.BlockSummary{},
			},
			wantErr: true,
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := datastore.PrintEntityHandler(tt.args.ctx, tt.args.entity)
			if (err != nil) != tt.wantErr {
				t.Errorf("PrintEntityHandler() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("PrintEntityHandler() got = %v, want %v", got, tt.want)
			}
		})
	}
}

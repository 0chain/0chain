...
type TokenPoolTransferResponse struct {
	...
	Value      tokens.Balance `json:"value,omitempty"`
...
type TokenPoolI interface {
	...
	SetBalance(value tokens.Balance)
	GetID() string
	...
	TransferTo(op TokenPoolI, value tokens.Balance, entity interface{}) (*state.Transfer, string, error)
	DrainPool(fromClientID, toClientID string, value tokens.Balance, entity interface{}) (*state.Transfer, string, error)

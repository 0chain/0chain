...
type TokenPoolTransferResponse struct {
	...
	Value      tokens.SAS `json:"value,omitempty"`
...
type TokenPoolI interface {
	GetBalance() tokens.SAS
	SetBalance(value tokens.SAS)
	GetID() string
	...
	TransferTo(op TokenPoolI, value tokens.SAS, entity interface{}) (*state.Transfer, string, error)
	DrainPool(fromClientID, toClientID string, value tokens.SAS, entity interface{}) (*state.Transfer, string, error)

...
func (c *Chain) SetupNodeHandlers() {
	http.HandleFunc(coreEndpoint.ListMiners, common.Recover(c.GetMinersHandler))
	http.HandleFunc(coreEndpoint.ListSharders, common.Recover(c.GetShardersHandler))
}
...
func setupX2MRequestors() {
	...
	MinerNotarizedBlockRequestor = node.RequestEntityHandler(minerEndpoint.AnyServiceToMinerGetNotarizedBlock, options, blockEntityMetadata)

	options = &node.SendOptions{Timeout: node.TimeoutLargeMessage, CODEC: node.CODEC_MSGPACK, Compress: true}
	blockStateChangeEntityMetadata := datastore.GetEntityMetadata("block_state_change")
	BlockStateChangeRequestor = node.RequestEntityHandler(minerEndpoint.AnyServiceToMinerGetStateChange, options, blockStateChangeEntityMetadata)

	stateNodesEntityMetadata := datastore.GetEntityMetadata("state_nodes")
	StateNodesRequestor = node.RequestEntityHandler(coreEndpoint.AnyServiceToAnyServiceGetNodes, options, stateNodesEntityMetadata)
}
...
func setupX2SRequestors() {
	...
	LatestFinalizedMagicBlockRequestor = node.RequestEntityHandler(coreEndpoint.GetLatestFinalizedMagicBlock, options, blockEntityMetadata)
	...
	FBRequestor = node.RequestEntityHandler(sharderEndpoint.AnyServiceToSharderGetBlock, &opts,
		datastore.GetEntityMetadata("block"))
}

func SetupX2XResponders(c *Chain) {
	http.HandleFunc(coreEndpoint.AnyServiceToAnyServiceGetNodes, common.N2NRateLimit(node.ToN2NSendEntityHandler(StateNodesHandler)))
	http.HandleFunc(coreEndpoint.AnyServiceToAnyServiceGetBlockStateChange, common.N2NRateLimit(node.ToN2NSendEntityHandler(c.BlockStateChangeHandler)))
}

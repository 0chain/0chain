...
func (c *Chain) getState(clientState util.MerklePatriciaTrieI, clientID string) (*state.State, error) {
	...
	ss, err := clientState.GetNodeValue(util.Path(clientID), s)
	if err != nil {
	...
	if val, ok := ss.(*state.State); !ok {
		return s, common.NewError("getState", "wrong MPT value type")
	} else {
		s = val
	}
	return s, nil
...
func (c *Chain) GetState(b *block.Block, clientID string) (*state.State, error) {
	...
	st := &state.State{}
	ss, err := b.ClientState.GetNodeValue(util.Path(clientID), st)
	if err != nil {
		if !b.IsStateComputed() {

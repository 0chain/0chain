 ...
 */
func (c *Chain) transferAmount(sctx bcstate.StateContextI, fromClient, toClient datastore.Key, amount int64) error {
	if amount == 0 {
		return nil
	}
...
func (c *Chain) mintAmount(sctx bcstate.StateContextI, toClient datastore.Key, amount int64) error {
	if amount == 0 {
		return nil
	}
	b := sctx.GetBlock()

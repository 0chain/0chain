...
func TestBlock_VerificationTicketsSize(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:   "OK",
			fields: fields{VerificationTickets: b.VerificationTickets},
			want:   2,
		},
	}
	for _, tt := range tests {
...
func TestBlock_Clear(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			b.Clear()
			old.PrevBlock = nil
			if !assert.Equal(t, &old, b) {
				t.Errorf("Clear(), got = %v, want = %v", b, &old)
			}
		})
	}
}
...
func TestBlock_UnknownTickets(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:   "OK",
			fields: fields{VerificationTickets: tickets},

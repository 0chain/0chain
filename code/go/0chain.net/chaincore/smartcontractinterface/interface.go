...
func NewSC(id string) *SmartContract {
	restHandlers := make([]map[string]SmartContractRestHandler, 1)
	restHandlers[0] = make(map[string]SmartContractRestHandler)
...
type SmartContractInterface interface {
	...
	GetRestPoints(version int) map[string]SmartContractRestHandler

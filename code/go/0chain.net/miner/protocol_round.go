...
func (mc *Chain) updatePreviousBlockNotarization(ctx context.Context, b *block.Block, pr *Round) error {
	...
	err := mc.verifyBlockNotarizationWorker.Run(ctx, func() error {
		...
		if err := mc.VerifyNotarization(ctx, pb, b.GetPrevBlockVerificationTickets()); err != nil {
...
func (mc *Chain) MergeNotarization(ctx context.Context, r *Round, b *block.Block, vts []*block.VerificationTicket) error {
	return mc.BlockTicketsVerifyWithLock(ctx, b.Hash, func() error {
		...
		if err := mc.VerifyTickets(ctx, b.Hash, vts, r.GetRoundNumber(), nil); err != nil {
			return err
		}
	...
	if rn > 1 {
		...
		if pr != nil {
			...
			if pnb != nil && pnb.IsBlockNotarized() {
				...
				go mc.SendNotarization(context.Background(), pnb)
			}
		}
	}
...
func (mc *Chain) LoadMagicBlocksAndDKG(ctx context.Context) {
	...
	var (
		...
		latestMB *block.MagicBlock // tracks the latest MB loaded for emergency recovery
		err      error
	)

	// Schedule emergency recovery at exit (if enabled in config).
	// latestMB will be set to whichever MB was successfully loaded.
	defer func() {
		mc.scheduleEmergencyRecovery(ctx, latestMB)
	}()
	...
	// Register nodes from the last 3 MBs so the miner can validate blocks/LFBs
	// generated under a recently-active MB. UpdateMagicBlock only keeps current +
	// previous MB miners (RegisterNodes replaces the entire map). During startup,
	// the stored LFB may have been generated by a miner from the MB before that.
	if finalMB != nil {
		const maxMBsToLoad = 3
		var allNodes []*node.Node
		for mbNum := finalMB.MagicBlockNumber; mbNum > 0 && mbNum > finalMB.MagicBlockNumber-int64(maxMBsToLoad); mbNum-- {
			mb, loadErr := LoadMagicBlock(ctx, strconv.FormatInt(mbNum, 10))
			if loadErr != nil || mb == nil {
				continue
			}
			for _, mn := range mb.Miners.CopyNodes() {
				if err := node.Setup(mn); err == nil {
					allNodes = append(allNodes, mn)
				}
			}
			for _, sh := range mb.Sharders.CopyNodes() {
				if err := node.Setup(sh); err == nil {
					allNodes = append(allNodes, sh)
				}
			}
		}
		if len(allNodes) > 0 {
			node.RegisterNodes(allNodes)
			logging.Logger.Info("load_mbs_and_dkg -- registered nodes from recent MBs",
				zap.Int("total_nodes", len(allNodes)),
				zap.Int64("latest_mb", finalMB.MagicBlockNumber))
		}
	}
}

...
func TestMain(m *testing.M) {
	...
	config.Configuration().ChainConfig = &event.TestConfig{
		conf: &event.TestConfigData{DbsSettings: dbSetting},
	}

	gEventDB = &event.EventDb{
		Store:         postgresql.New(gormDB),
		eventsChannel: make(chan event.BlockEvents, 1),
		settings:      dbSetting,
	}

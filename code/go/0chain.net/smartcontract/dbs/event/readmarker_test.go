...
func TestReadMarkersPaginated(t *testing.T) {
	...
	t.Run("get all readmarker with pagination limit", func(t *testing.T) {
		...
		for i, rm := range rms {
			...
			want := ReadMarker{TransactionID: transactionHash, BlobberID: "blobberID 0", ClientID: "someClientID", AllocationID: strconv.Itoa(i), AuthTicket: strconv.Itoa(i), BlockNumber: int64(i), ReadSize: int64(i)}
			want.ID = rm.ID
	...
	t.Run("get all readmarker with pagination offset and limit", func(t *testing.T) {
		...
		for i, rm := range rms {
			...
			want := ReadMarker{TransactionID: transactionHash, BlobberID: "blobberID 0", ClientID: "someClientID", AllocationID: strconv.Itoa(i + 5), AuthTicket: strconv.Itoa(i + 5), BlockNumber: int64(i + 5), ReadSize: int64(i + 5)}
			want.ID = rm.ID
			want.CreatedAt = rm.CreatedAt
			want.UpdatedAt = rm.UpdatedAt
			assert.Equal(t, want, rm, "RM was not correct")
	...
	t.Run("get all readmarker with pagination descending", func(t *testing.T) {
		...
		for i, rm := range rms {
			...
			want := ReadMarker{TransactionID: transactionHash, BlobberID: "blobberID 9", ClientID: "someClientID", AllocationID: strconv.Itoa(9 - i), AuthTicket: strconv.Itoa(9 - i), BlockNumber: int64(9 - i), ReadSize: int64(9 - i)}
			want.ID = rm.ID
			want.CreatedAt = rm.CreatedAt
			want.UpdatedAt = rm.UpdatedAt
			assert.Equal(t, want, rm, "RM was not correct")
...
func insertMultipleReadMarker(t *testing.T, eventDb *EventDb) {
	for j := 0; j < 10; j++ {
		...
		for i := 0; i < 10; i++ {
			...
			err = eventDb.addOrOverwriteReadMarker(ReadMarker{TransactionID: transactionHash, BlobberID: blobberID, ClientID: "someClientID", AllocationID: strconv.Itoa(i), AuthTicket: strconv.Itoa(i), BlockNumber: int64(i), ReadSize: int64(i)})

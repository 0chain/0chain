...
func TestReadMarkersPaginated(t *testing.T) {
	...
	t.Run("get all readmarker with allocationID", func(t *testing.T) {
		rms, err := eventDb.GetReadMarkersFromQueryPaginated(ReadMarker{AllocationID: "1"}, LimitData{-1, -1, false})
	...
	t.Run("get all readmarker with authticket", func(t *testing.T) {
		rms, err := eventDb.GetReadMarkersFromQueryPaginated(ReadMarker{AuthTicket: "1"}, LimitData{-1, -1, false})
		assert.NoError(t, err)
		for _, rm := range rms {
	...
	t.Run("get all readmarker with pagination limit", func(t *testing.T) {
		rms, err := eventDb.GetReadMarkersFromQueryPaginated(ReadMarker{}, LimitData{0, 10, false})
		assert.NoError(t, err)
		for i, rm := range rms {
	...
	t.Run("get all readmarker with pagination offset and limit", func(t *testing.T) {
		rms, err := eventDb.GetReadMarkersFromQueryPaginated(ReadMarker{}, LimitData{5, 5, false})
		assert.NoError(t, err)
		for i, rm := range rms {
	...
	t.Run("get all readmarker with pagination descending", func(t *testing.T) {
		rms, err := eventDb.GetReadMarkersFromQueryPaginated(ReadMarker{}, LimitData{0, 10, true})
		assert.NoError(t, err)
		for i, rm := range rms {

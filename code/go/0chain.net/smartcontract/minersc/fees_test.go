...
func newMiner(t *testing.T, msc *MinerSmartContract, now, ns int64,
	val int64, balances cstate.StateContextI) (mn *miner) {
...
func newSharder(t *testing.T, msc *MinerSmartContract, now, ns int64,
	val int64, balances cstate.StateContextI) (sh *sharder) {
...
func computeMinerPayments(gn *GlobalNode, msc *MinerSmartContract, b *block.Block) int64 {
...
func computeShardersPayments(gn *GlobalNode, msc *MinerSmartContract, b *block.Block) int64 {
	blockReward := gn.BlockReward
...
func Test_payFees(t *testing.T) {
	...
	t.Run("pay fees -> view change", func(t *testing.T) {
		...
		for _, sh := range sharders {
			if existInDelegatesOfNodes(sh.delegate.id, blockSharders) {
				shP := computeShardersPayments(gn, msc, b) / int64(len(blockSharders))
				assert.Equal(t,
	...
	t.Run("pay fees -> no fees", func(t *testing.T) {
		...
		blockSharders, err := msc.getBlockSharders(b, balances)
		require.NoError(t, err)
		sharderPayments := computeShardersPayments(gn, msc, b) / int64(len(blockSharders))
		for _, sh := range sharders {
			if existInDelegatesOfNodes(sh.delegate.id, blockSharders) {
				assert.Equal(t,
					balances.balances[sh.delegate.id],
	...
	// reset all balances
	balances.balances = make(map[string]int64)

...
func TestQuickFixDuplicateHosts(t *testing.T) {
	...
	assert.EqualValues(t, "invalid n2nhost: ''", quickFixDuplicateHosts(node("", "", "", 0), nodes()).Error())
	assert.EqualValues(t, "invalid n2nhost: 'localhost'", quickFixDuplicateHosts(node("", "localhost", "", 0), nodes()).Error())
	assert.EqualValues(t, "invalid n2nhost: '127.0.0.1'", quickFixDuplicateHosts(node("", "127.0.0.1", "", 0), nodes()).Error())
	...
	assert.EqualValues(t, "n2nhost:port already exists: 'abc.com:0'", quickFixDuplicateHosts(node("abc", "abc.com", "", 0), nodes()).Error())
	assert.NoError(t, quickFixDuplicateHosts(node("", "abc.com", "", 1), nodes()))
	assert.EqualValues(t, "host:port already exists: 'lmn.com:0'", quickFixDuplicateHosts(node("mn", "lmn.com", "", 0), nodes()).Error())

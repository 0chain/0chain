...
func GetEndpoints(rh rest.RestHandlerI) []rest.Endpoint {
	...
	return []rest.Endpoint{
		rest.MakeEndpoint(miner+"/global-settings", mrh.getGlobalSettings),
		rest.MakeEndpoint(miner+"/node-pool", mrh.getNodePool),
		rest.MakeEndpoint(miner+"/user-pools", mrh.getUserPools),
		rest.MakeEndpoint(miner+"/miners", mrh.getMinerList),
		rest.MakeEndpoint(miner+"/get-miners-stats", mrh.getMinersStats),
		rest.MakeEndpoint(miner+"/get-miners-stake", mrh.getMinersStake),
		rest.MakeEndpoint(miner+"/sharders", mrh.getSharderList),
		rest.MakeEndpoint(miner+"/sharders-stats", mrh.getShardersStats),
		rest.MakeEndpoint(miner+"/sharders-stake", mrh.getShardersStake),
		rest.MakeEndpoint(miner+"/sharder-keeps", mrh.getSharderKeepList),
		rest.MakeEndpoint(miner+"/phase", mrh.getPhase),
		rest.MakeEndpoint(miner+"/dkgs", mrh.getDkgList),
		rest.MakeEndpoint(miner+"/mpks", mrh.getMpksList),
		rest.MakeEndpoint(miner+"/group-share-or-signs", mrh.getGroupShareOrSigns),
		rest.MakeEndpoint(miner+"/magic-block", mrh.getMagicBlock),
		rest.MakeEndpoint(miner+"/events", mrh.getEvents),
		rest.MakeEndpoint(miner+"/node-stat", mrh.getNodeStat),
		rest.MakeEndpoint(miner+"/node-pool-stat", mrh.getNodePoolStat),
		rest.MakeEndpoint(miner+"/config", mrh.getConfigs),
		rest.MakeEndpoint(miner+"/miner-geolocations", mrh.getMinerGeolocations),
		rest.MakeEndpoint(miner+"/sharder-geolocations", mrh.getSharderGeolocations),
	}
}

...
type Client struct {
	...
	balance int64                      // client wallet balance
}

func newClient(balance int64, balances cstate.StateContextI) (
	client *Client) {
...
func setConfig(t *testing.T, balances cstate.StateContextI) (
	...
	gn.KPercent = 0.75   // %
	gn.LastRound = 0
	gn.MaxStake = tokens.ZCNToSAS(100.0)
	gn.MinStake = tokens.ZCNToSAS(0.01)
	gn.RewardRate = 1.0
	gn.ShareRatio = 0.10
	gn.BlockReward = tokens.ZCNToSAS(0.7)
	gn.MaxCharge = 0.5 // %
	gn.Epoch = 15e6    // 15M
	gn.RewardDeclineRate = 0.1
	gn.MaxMint = tokens.ZCNToSAS(4e6)
	gn.Minted = 0

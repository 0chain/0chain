...
func Test_toSeconds(t *testing.T) {
	require.Equal(t, common.Timestamp(1),
...
func Test_lockRequest_decode(t *testing.T) {
	...
	require.Equal(t, "pool_hex", lr.PoolID)
}
...
func Test_addRequest_validate(t *testing.T) {
	...
	require.EqualError(t, ar.validate(10, conf), "entry description is too long")
	ar.Description = "short desc."

	ar.StartTime = 1
	require.EqualError(t, ar.validate(10, conf), "vesting starts before now")
	ar.StartTime = 20

	require.EqualError(t, ar.validate(10, conf), "vesting duration is too short")
	ar.Duration = 20 * time.Hour
	require.EqualError(t, ar.validate(10, conf), "vesting duration is too long")
	ar.Duration = 1 * time.Minute

	require.EqualError(t, ar.validate(10, conf), "no destinations")
	ar.Destinations = destinations{
		&destination{ID: "one", Amount: 10},
		&destination{ID: "two", Amount: 20},
		&destination{ID: "three", Amount: 30},
	}
	require.EqualError(t, ar.validate(10, conf), "too many destinations")
	ar.Destinations = destinations{
		&destination{ID: "one", Amount: 10},
		&destination{ID: "two", Amount: 20},
	}

	require.NoError(t, ar.validate(10, conf))
	ar.StartTime = 0
	require.NoError(t, ar.validate(10, conf))
}
...
func Test_vestingPool(t *testing.T) {
	...
	require.NotNil(t, vp)
	require.NotNil(t, vp.TokenPool)

	require.Equal(t, vp.Description, ar.Description)
	require.Equal(t, vp.StartTime, ar.StartTime)
	require.Equal(t, vp.ExpireAt, ar.StartTime+toSeconds(ar.Duration))
	require.Equal(t, vp.Destinations, ar.Destinations)
	vp.Balance = 40

	var vpd = new(vestingPool)
	require.NoError(t, vpd.Decode(vp.Encode()))
	require.Equal(t, vp, vpd)

	var inf = vpd.info(11)
	require.Equal(t, vp.Description, inf.Description)
	require.Equal(t, vp.StartTime, inf.StartTime)
	require.Equal(t, vp.ExpireAt, inf.ExpireAt)
	require.Equal(t, []*destInfo{
		{ID: "one", Wanted: 10, Earned: 5, Vested: 0, Last: 10},
		{ID: "two", Wanted: 20, Earned: 10, Vested: 0, Last: 10},
	}, inf.Destinations) // TODO
	require.Equal(t, state.Balance(40), inf.Balance)
	require.Equal(t, state.Balance(10), inf.Left)
}
...
func TestVestingSmartContract_getPoolBytes_getPool(t *testing.T) {
	...
	require.Equal(t, string(vp.Encode()), string(poolb))
	got, err = vsc.getPool(poolKey(vsc.ID, txHash), balances)
	require.NoError(t, err)
	require.Equal(t, vp, got)
}
...
func TestVestingSmartContract_add(t *testing.T) {
	...
	require.EqualError(t, err, `create_vesting_pool_failed: malformed request:`+
	...
	require.EqualError(t, err, `create_vesting_pool_failed: invalid request:`+
	...
	tx.ClientID = ""
	_, err = vsc.add(tx, mustEncode(t, &ar), balances)
	require.EqualError(t, err, `create_vesting_pool_failed: `+
	...
	require.NotZero(t, deco.ID)
	require.Equal(t, client.id, deco.ClientID)
	require.Equal(t, state.Balance(800e10), deco.Balance)

	// 7. client pools
	var cp *clientPools
	cp, err = vsc.getClientPools(client.id, balances)
	require.NoError(t, err)
	require.Equal(t, []string{deco.ID}, cp.Pools)
}
...
func TestVestingSmartContract_delete(t *testing.T) {
	...
	require.EqualError(t, err, "delete_vesting_pool_failed: invalid request:"+
		" invalid character '}' looking for beginning of value")
	...
	tx.ClientID = ""
	_, err = vsc.delete(tx, mustEncode(t, &dr), balances)
	require.EqualError(t, err, "delete_vesting_pool_failed: "+
	...
	require.NoError(t, err)
	require.Equal(t, `{"pool_id":"`+set.ID+`","action":"deleted"}`, resp)

	require.Zero(t, balances.tree[set.ID])
	require.Zero(t, balances.tree[clientPoolsKey(vsc.ID, client.id)])
}
...
func TestVestingSmartContract_stop(t *testing.T) {
	...
	require.EqualError(t, err, "stop_vesting_failed: malformed request:"+
		" invalid character '}' looking for beginning of value")

	// 2. destination = ""
	_, err = vsc.stop(tx, mustEncode(t, &sr), balances)
	require.EqualError(t, err, "stop_vesting_failed:"+
	...
	require.NoError(t, err)
	require.Equal(t, state.Balance(8e12), got.Balance)
...
func TestVestingSmartContract_unlock(t *testing.T) {
	...
	require.EqualError(t, err, "unlock_vesting_pool_failed: invalid request:"+
		" invalid character '}' looking for beginning of value")
	...
	lr.PoolID = "pool_hex"
	_, err = vsc.unlock(tx, mustEncode(t, &lr), balances)
	require.EqualError(t, err, "unlock_vesting_pool_failed: "+
		"can't get pool: value not present")
	...
	got, err = vsc.getPool(set.ID, balances)
	require.NoError(t, err)
	require.Equal(t, state.Balance(30), got.Balance)
}
...
func TestVestingSmartContract_trigger(t *testing.T) {
	...
	require.EqualError(t, err, "trigger_vesting_pool_failed: invalid request:"+
		" invalid character '}' looking for beginning of value")
	...
	lr.PoolID = "pool_hex"
	_, err = vsc.trigger(tx, mustEncode(t, &lr), balances)
	require.EqualError(t, err, "trigger_vesting_pool_failed: "+
		"can't get pool: value not present")
	...
	got, err = vsc.getPool(set.ID, balances)
	require.NoError(t, err)
	require.Equal(t, state.Balance(29000), got.Balance)
}

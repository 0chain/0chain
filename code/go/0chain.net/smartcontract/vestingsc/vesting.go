...
type destination struct {
	...
	Amount int64  `json:"amount"` // amount to vest for the destination (initial)
	Vested int64  `json:"vested"` // tokens already vested
...
func (d *destination) left() (left int64) {
...
func (d *destination) move(now common.Timestamp, moved int64) {
	d.Last = now
	if moved > 0 {
		d.Move = now
		d.Vested += moved
...
func (d *destination) unlock(now, end common.Timestamp, dry bool) (
	...
	amount = int64(float64(left) * ratio)
...
func (vp *vestingPool) want() (want int64) {
...
func (vp *vestingPool) moveToDest(vscKey, destID datastore.Key,
	value int64, balances chainstate.StateContextI) (
...
func (vp *vestingPool) excess() (amount int64) {
	var need int64
	for _, d := range vp.Destinations {
		need += d.left()
	}
...
type destInfo struct {
	...
	Wanted int64            `json:"wanted"` // wanted amount for entire period
	Earned int64            `json:"earned"` // can unlock
	Vested int64            `json:"vested"` // tokens already vested
...
type info struct {
	...
	Balance      int64            `json:"balance"`      // real pool balance
	Left         int64            `json:"left"`         // owner can unlock

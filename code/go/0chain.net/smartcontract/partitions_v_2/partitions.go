...
func (p *Partitions) Add(state state.StateContextI, item common2.PartitionItem) error {
...
func (p *Partitions) AddX(state state.StateContextI, item common2.PartitionItem) (int, error) {
	// duplicate item checking
	_, ok, err := p.getItemPartIndex(state, item.GetID())
	if err != nil {
		return -1, err
...
func (p *Partitions) add(state state.StateContextI, item common2.PartitionItem) (int, error) {
...
func (p *Partitions) Get(state state.StateContextI, id string, v common2.PartitionItem) (int, error) {
	...
	if !ok {
		return -1, common.NewError(common2.ErrItemNotFoundCode, id)
	}
...
func (p *Partitions) get(state state.StateContextI, partIndex int, id string, v common2.PartitionItem) error {
...
func (p *Partitions) UpdateItem(state state.StateContextI, it common2.PartitionItem) error {
	...
	if !ok {
		return common.NewError(common2.ErrItemNotFoundCode, it.GetID())
	}
...
func (p *Partitions) Update(state state.StateContextI, key string,
	...
	if !ok {
		return -1, common.NewError(common2.ErrItemNotFoundCode, key)
	}
...
func (p *Partitions) Remove(state state.StateContextI, id string) error {
	...
	if !ok {
		return common.NewError(common2.ErrItemNotFoundCode, id)
	}
...
func (p *Partitions) RemoveX(state state.StateContextI, id string) (*RemoveLocs, error) {
	...
	if !ok {
		return nil, common.NewError(common2.ErrItemNotFoundCode, id)
	}
...
func setPartitionItems(rtv []item, vs interface{}) error {
	...
	for i, v := range rtv {
		...
		pi, ok := reflect.New(vt).Interface().(common2.PartitionItem)
		if !ok {

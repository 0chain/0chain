...
func testGlobalNode(id string, maxMint, totalMint, minLock int64, apr float64, minLockP time.Duration, ownerId datastore.Key) *GlobalNode {
...
func testGlobalNodeStringTime(id string, maxMint, totalMint, minLock, apr float64, minLockP string, ownerId string) *GlobalNode {
	var gn = &GlobalNode{ID: id}
	gn.SimpleGlobalNode = &SimpleGlobalNode{
		MaxMint:     tokens.ZCNToSAS(maxMint),
		TotalMinted: tokens.ZCNToSAS(totalMint),
		MinLock:     tokens.ZCNToSAS(minLock),
		APR:         apr,
		OwnerId:     ownerId,
		Cost:        map[string]int{},
	}
...
func testInterestPool(sec time.Duration, balance int64) *interestPool {

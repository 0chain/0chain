		...
		*transaction.Transaction,
			...
			for j := 0; j < args.poolCount[i]; j++ {
				...
				for _, blobber := range blobbers {
					ap.Blobbers.add(&blobberPool{
						...
						Balance:   ap.Balance / int64(bCount*args.poolCount[i]),
					})
				}
				wp.Pools.add(&ap)
			}
		...
		balances.On(
			...
			mock.MatchedBy(func(cp *challengePool) bool {
				...
				return cp.Balance/10 == int64(newFunds/10) // ignore type cast errors
			}),
		).Return("", nil).Once()
...
func TestStorageSmartContract_updateAllocationRequest(t *testing.T) {
	...
	for _, b := range blobs {
		...
		require.NoError(t, err)
		blob.Terms.WritePrice = tokens.ZCNToSAS(1.8)
		blob.Terms.ReadPrice = tokens.ZCNToSAS(0.8)

...
func (cp *challengePool) moveToValidators(sscKey string, reward float64,
	...
	if len(validators) == 0 || reward == 0.0 {
		return nil // nothing to move, or nothing to move to
	}

	var oneReward = reward / float64(len(validators))

	for i, sp := range vSPs {
		if float64(cp.Balance) < oneReward {
			return fmt.Errorf("not enough tokens in challenge pool: %v < %v", cp.Balance, oneReward)
		}
		err := sp.DistributeRewards(currency.Coin(oneReward), validators[i], spenum.Validator, balances)
		if err != nil {
			return fmt.Errorf("moving to validator %s: %v",
				validators[i], err)
		}
		cp.ZcnPool.Balance -= currency.Coin(oneReward)
	}
	return nil
}

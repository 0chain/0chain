...
func GetEndpoints(rh rest.RestHandlerI) []rest.Endpoint {
	...
	return []rest.Endpoint{
		rest.MakeEndpoint(storage+"/blobber-count", srh.getBlobberCount),
		rest.MakeEndpoint(storage+"/blobber", srh.getBlobber),
		rest.MakeEndpoint(storage+"/blobbers", srh.getBlobbers),
		rest.MakeEndpoint(storage+"/blobber-total-stakes", srh.getBlobberTotalStakes),
		...
		rest.MakeEndpoint(storage+"/write-marker", srh.getWriteMarker),
		...
		rest.MakeEndpoint(storage+"/latest-read-marker", srh.getLatestReadMarker),
		rest.MakeEndpoint(storage+"/read-markers", srh.getReadMarkers),
		rest.MakeEndpoint(storage+"/read-marker-count", srh.getReadMarkersCount),
		rest.MakeEndpoint(storage+"/write-markers", srh.getWriteMarkers),
		rest.MakeEndpoint(storage+"/validator", srh.getValidator),
		rest.MakeEndpoint(storage+"/open-challenges", srh.getOpenChallenges),
		rest.MakeEndpoint(storage+"/challenge", srh.getChallenge),
		rest.MakeEndpoint(storage+"/stake-pool-stat", srh.getStakePoolStat),
		rest.MakeEndpoint(storage+"/user-stake-pool-stat", srh.getUserStakePoolStat),
		rest.MakeEndpoint(storage+"/block-by-hash", srh.getBlockByHash),
		rest.MakeEndpoint(storage+"/blocks", srh.getBlocks),
		rest.MakeEndpoint(storage+"/total-stored-data", srh.getTotalData),
		rest.MakeEndpoint(storage+"/storage-config", srh.getConfig),
		rest.MakeEndpoint(storage+"/read-pool-stat", srh.getReadPoolStat),
		rest.MakeEndpoint(storage+"/read-pool-alloc-blobber-stat", srh.getReadPoolAllocBlobberStat),
		rest.MakeEndpoint(storage+"/write-pool-stat", srh.getWritePoolStat),
		rest.MakeEndpoint(storage+"/write-pool-alloc-blobber-stat", srh.getWritePoolAllocBlobberStat),
		rest.MakeEndpoint(storage+"/challenge-pool-stat", srh.getChallengePoolStat),
		rest.MakeEndpoint(storage+"/alloc-written-size", srh.getWrittenAmount),
		rest.MakeEndpoint(storage+"/alloc-read-size", srh.getReadAmount),
		rest.MakeEndpoint(storage+"/alloc-write-marker-count", srh.getWriteMarkerCount),
		rest.MakeEndpoint(storage+"/collected-reward", srh.getCollectedReward),
		rest.MakeEndpoint(storage+"/blobber-ids", srh.getBlobberIdsByUrls),

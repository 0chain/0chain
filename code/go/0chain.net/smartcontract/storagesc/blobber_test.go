...
func Test_flow_reward(t *testing.T) {
	...
	t.Run("challenge pass", func(t *testing.T) {
		...
		var sp *stakePool
		sp, err = ssc.getStakePool(b3.id, conf, balances)
		require.NoError(t, err)
		...
		for i := int64(0); i < 10+1; i++ {
			...
			// validators reward
			for _, val := range valids {
				_, err = ssc.getStakePool(val.id, conf, balances)
				require.NoError(t, err)
			}
...
func Test_flow_penalty(t *testing.T) {
	...
	t.Run("challenge penalty", func(t *testing.T) {
		...
		var sp *stakePool
		sp, err = ssc.getStakePool(b4.id, conf, balances)
		require.NoError(t, err)
...
func Test_flow_no_challenge_responses_finalize(t *testing.T) {
	...
	t.Run("challenges without a response", func(t *testing.T) {
		...
		for _, b := range blobs {
			...
			var sp *stakePool
			sp, err = ssc.getStakePool(b.id, conf, balances)
			require.NoError(t, err)
		...
		for _, val := range valids {
			var vsp *stakePool
			vsp, err = ssc.getStakePool(val.id, conf, balances)
			require.NoError(t, err)

...
func transferReward(
	...
	if zcnPool.Balance < value {
		return 0, zchainErrors.Newf("", "not enough tokens in pool %s: %d < %d",
	...
	for _, payment := range payments {
		...
		if err != nil {
			return 0, zchainErrors.Newf("", "transferring tokens challenge_pool(%s) -> "+
				"stake_pool_holder(%s): %v", zcnPool.ID, payment.to, err)
		}
		if err = balances.AddTransfer(transfer); err != nil {
			return 0, zchainErrors.Newf("", "adding transfer: %v", err)
		}
	}
...
func mintReward(
	...
	for _, payment := range payments {
		if err := balances.AddMint(&state.Mint{
			...
			return zchainErrors.Newf("", "minting rewards: %v", err)
		}
	}
	return nil
}
...
func getPayments(sp *stakePool, value float64) ([]payment, float64, error) {
	...
	if len(sp.Pools) == 0 {
		return nil, 0, zchainErrors.Newf("", "no stake pools to move tokens to")
	}

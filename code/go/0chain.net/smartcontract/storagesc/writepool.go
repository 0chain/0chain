...
func (ssc *StorageSmartContract) writePoolLock(
	...
	i, _ := txn.Value.Int64()
	balances.EmitEvent(event.TypeStats, event.TagLockWritePool, txn.ClientID, event.WritePoolLock{
		Client:       txn.ClientID,
...
func (ssc *StorageSmartContract) writePoolUnlock(
	...
	balances.EmitEvent(event.TypeStats, event.TagUnlockWritePool, txn.ClientID, event.WritePoolLock{
		Client:       txn.ClientID,

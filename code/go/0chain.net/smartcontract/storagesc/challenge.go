...
func (sc *StorageSmartContract) blobberReward(t *transaction.Transaction,
	...
	if back > 0 {
		...
		err = cp.moveToWritePool(alloc, blobAlloc.BlobberID, until, wp, int64(back))
		if err != nil {
			return fmt.Errorf("moving partial challenge to write pool: %v", err)
		}
		alloc.MovedBack += int64(back)
		blobAlloc.Returned += int64(back)
		// save write pool
	...
	err = sp.DistributeRewards(int64(blobberReward), blobAlloc.BlobberID, spenum.Blobber, balances)
	if err != nil {
		return fmt.Errorf("can't move tokens to blobber: %v", err)
	}

	blobAlloc.ChallengeReward += int64(blobberReward)
	...
	alloc.MovedToValidators += int64(validatorsReward)
...
func (sc *StorageSmartContract) blobberPenalty(t *transaction.Transaction,
	...
	// move back to write pool
	var until = alloc.Until()
	err = cp.moveToWritePool(alloc, blobAlloc.BlobberID, until, wp, int64(move))
	if err != nil {
		return fmt.Errorf("moving failed challenge to write pool: %v", err)
	}
	alloc.MovedBack += int64(move)
	blobAlloc.Returned += int64(move)

	// blobber stake penalty
	if conf.BlobberSlash > 0 && move > 0 &&
		int64(conf.BlobberSlash*move) > 0 {

		var slash = int64(conf.BlobberSlash * move)

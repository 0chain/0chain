...
func (sc *StorageSmartContract) blobberReward(_ *transaction.Transaction,
...
func (sc *StorageSmartContract) blobberPenalty(_ *transaction.Transaction,
	...
	if conf.BlobberSlash.Cmp(zbig.ZeroBigRat) > 0 && move > 0 && slash > 0 {
		// load stake pool
		var sp *stakePool
		if sp, err = sc.getStakePool(spenum.Blobber, blobAlloc.BlobberID, balances); err != nil {
...
func (sc *StorageSmartContract) verifyChallenge(t *transaction.Transaction,
	...
	if pass && fresh {
		...
		if blobber.RewardPartition.StartRound != rewardRound ||
			...
			totalData, _ := sizeInGB(blobber.SavedData).Float64()

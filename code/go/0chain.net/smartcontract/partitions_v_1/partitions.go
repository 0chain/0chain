...
func (p *Partitions) Add(state state.StateContextI, item common2.PartitionItem) error {
	...
	if ok {
		return common.NewError(common2.ErrItemExistCode, item.GetID())
	}
...
func (p *Partitions) Update(state state.StateContextI, key string, f func(data []byte) ([]byte, error)) (int, error) {

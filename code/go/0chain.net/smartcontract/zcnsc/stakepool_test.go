...
// func Test_AddToDelegatePool_Success(t *testing.T) {
// 	ctx := MakeMockStateContext()
// 	sc := CreateZCNSmartContract()

// 	// Mock global node for GetGlobalNode
// 	gn := &GlobalNode{
// 		ZCNSConfig : &ZCNSConfig { MinStakeAmount: 100,
// 		MaxStakeAmount: 1000,
// 		MaxDelegates:   10,
// 		MinStakePerDelegate: 100,},
// 	}
// 	SetGlobalNode(ctx, gn)

// 	tr := CreateTransaction("txn_id", "add_to_delegate_pool", []byte("input"), ctx)
// 	resp, err := sc.AddToDelegatePool(tr, []byte("input"), ctx)
// 	require.NoError(t, err)
// 	require.NotEmpty(t, resp)
// }

// func Test_AddToDelegatePool_GetGlobalNodeError(t *testing.T) {
// 	ctx := MakeMockStateContext()
// 	sc := CreateZCNSmartContract()

// 	expectedErr := errors.New("error getting global node")
// 	SetErrorOnGetGlobalNode(ctx, expectedErr)

// 	tr := CreateTransaction("txn_id", "add_to_delegate_pool", []byte("input"), ctx)
// 	resp, err := sc.AddToDelegatePool(tr, []byte("input"), ctx)
// 	require.Error(t, err)
// 	require.Empty(t, resp)
// 	require.EqualError(t, err, common.NewErrorf("add-to-delegate-pool-failed", "failed to get global node error: %v", expectedErr).Error())
// }

// func Test_DeleteFromDelegatePool_Success(t *testing.T) {
// 	ctx := MakeMockStateContext()
// 	sc := CreateZCNSmartContract()

// 	tr := CreateTransaction("txn_id", "delete_from_delegate_pool", []byte("input_data"), ctx)
// 	resp, err := sc.DeleteFromDelegatePool(tr, []byte("input_data"), ctx)
// 	require.NoError(t, err)
// 	require.NotEmpty(t, resp)
// }
...
// 		resp, err := mockGetGlobalNode(ctx)
// 		require.NotNil(t, resp)
// 		require.NoError(t, err)

// 		resp1, err1 := zcn.AddToDelegatePool(nil, nil, ctx)

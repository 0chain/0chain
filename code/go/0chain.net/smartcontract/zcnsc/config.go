...
func loadGlobalNode() (conf *ZCNSConfig) {
	conf = new(ZCNSConfig)
	conf.MinMintAmount = tokens.Balance(cfg.GetInt(postfix(MinMintAmount)))
	conf.MinBurnAmount = tokens.Balance(cfg.GetInt64(postfix(MinBurnAmount)))
	conf.MinStakeAmount = tokens.Balance(cfg.GetInt64(postfix(MinStakeAmount)))
	...
	conf.MaxFee = tokens.Balance(cfg.GetInt64(postfix(MaxFee)))
